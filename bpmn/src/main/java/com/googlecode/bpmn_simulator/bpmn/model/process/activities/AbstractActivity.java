...
public abstract class AbstractActivity
	...
	private final Tokens incomingTokens = new Tokens();
	private final Tokens outgoingTokens = new Tokens();

	public AbstractActivity(final String id, final String name) {
		super(id, name);
	}

	protected final Tokens getIncomingTokens() {
		return incomingTokens;
	}

	protected final Tokens getOutgoingTokens() {
		return outgoingTokens;
	}

	@Override
	public Tokens getTokens() {
		final Tokens tokens = super.getTokens();
	...
	protected void forwardTokenFromOutgoing(final Token token) {
		final Instance instance = token.getInstance();
		tokenForwardToNextElement(token, instance.getParentInstance());
		instance.remove();
	}
	...
	public boolean hasIncomingTokens() {
		final Tokens incomingTokens = getIncomingTokens();
		return (incomingTokens != null) && !incomingTokens.isEmpty();
	}

	public boolean hasOutgoingTokens() {
		final Tokens outgoingTokens = getOutgoingTokens();
